...
<Section name="register">
    <Register
        divClass="w-full bg-white rounded-lg shadow-lg dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-black dark:border-gray-700"
    >
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            <form class="flex flex-col space-y-6" method="post" use:enhance>
                <h3
                    class="text-xl font-extrabold text-black dark:text-white p-0"
                >
                    create an account
                </h3>
                ...
                {#if form?.usernameInvalid}
                    <Helper color="red">username is missing</Helper>
                {/if}
                <Label class="space-y-2 text-black dark:text-white">
                    <span>name</span>
                ...
                {#if form?.passwordInvalid}
                    <Helper color="red">password is missing</Helper>
                {/if}
                <Label class="space-y-2 text-black dark:text-white">
                ...
                {#if form?.unknownError}
                    <Helper color="red">an error occurred</Helper>
                {/if}
